...
export function RegisterForm({ programs }: RegisterFormProps) {
  ...
  return <form action={action} className="auth-form registration-form" noValidate onSubmit={validate} onInput={clearClientError}>
    ...
    <button className="button button-primary button-full" disabled={pending || !programs.length}>{pending ? <LoaderCircle className="spin" size={18}/> : <ArrowRight size={18}/>}Create account</button>
  </form>;
}
