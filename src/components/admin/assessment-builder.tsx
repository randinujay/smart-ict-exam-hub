          ...
          <article className="builder-question" key={question.id} data-question-id={question.id}>
            ...
            {expanded === question.id && <div className="builder-question-body">
              <div className="admin-form-grid">
                ...
                <label className="field full"><span>Prompt</span><textarea disabled={questionsLocked} rows={3} placeholder="Enter the question" value={question.prompt} onChange={(event) => patchQuestion(question.id, { prompt: event.target.value })} /></label>
              </div>
              {["single_choice", "multiple_choice", "true_false"].includes(question.type) && <><p className="option-builder-hint">Select the letter beside each correct answer.</p><div className="option-builder">{question.options.map((option) => {
                ...
                }} aria-label={`Mark option ${option.key} as correct`} aria-pressed={correct} title="Mark as correct"><Check size={15} />{option.key}</button><input disabled={questionsLocked || question.type === "true_false"} placeholder={`Option ${option.key}`} value={option.text} onChange={(event) => patchOption(question.id, option.id, event.target.value)} /></div>;
              })}</div></>}
