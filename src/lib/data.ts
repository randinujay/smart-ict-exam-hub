...
export interface AdminAssessmentEditorData {
  assessment: Assessment;
  hasAttempts: boolean;
}

export const getAdminAssessment = cache(async (assessmentId: string): Promise<AdminAssessmentEditorData | null> => {
  if (isDemoMode()) {
    const assessment = demoAssessments.find((item) => item.id === assessmentId);
    return assessment ? { assessment, hasAttempts: false } : null;
  }
  if (!isSupabaseConfigured()) throw new Error("Supabase configuration is missing.");
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("get_admin_assessment", {
    p_assessment_id: assessmentId,
  });
  if (error) {
    console.error("get_admin_assessment failed", error);
    throw new Error("The assessment editor could not be loaded.");
  }
  return data ? data as AdminAssessmentEditorData : null;
});
