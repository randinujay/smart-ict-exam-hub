...
function validate(payload: Payload) {
  ...
  if (!Array.isArray(payload.batchIds) || !payload.batchIds.length) return "Choose at least one class.";
  if (payload.batchIds.some((id) => !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(id)))) return "One or more selected classes are invalid.";
...
async function save(request: Request, mode: "create" | "update") {
  ...
  try {
    ...
    payload.batchIds = [...new Set((Array.isArray(payload.batchIds) ? payload.batchIds : []).map(String))];
    payload.programIds = [];
    payload.studentIds = [];
