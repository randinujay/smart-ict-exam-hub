...
export default async function AdminPaymentsPage() {
  ...
  return <div>
    ...
    <details className="admin-create-panel"><summary><BadgeDollarSign size={17} />Record payment</summary><form action={markPaymentAction} className="admin-form-grid">
      ...
      <ClassSelectFields programs={data.programs} academicBatches={data.academicBatches} batches={data.batches} />
    ...
    <section className="results-table-card admin-table-card">{data.payments.length ? <div className="responsive-table"><table><thead><tr><th>Student</th><th>Month</th><th>Class</th><th>Amount</th><th>Status</th><th>Actions</th></tr></thead><tbody>
      {data.payments.map((payment) => { const student = data.students.find((item) => item.id === payment.studentId); const assignedClass = data.batches.find((item) => item.id === payment.batchId); return <tr key={payment.id}>
        ...
        <td>{new Date(payment.billingMonth).toLocaleDateString("en-LK", { month: "long", year: "numeric" })}</td><td>{assignedClass?.className}</td><td>LKR {payment.amount.toLocaleString()}</td><td><StatusBadge tone={payment.status === "paid" || payment.status === "waived" ? "success" : "warning"}>{payment.status}</StatusBadge></td>
        <td><div className="table-actions"><details className="inline-editor"><summary><Pencil size={15} />Edit</summary><form action={updatePaymentAction} className="admin-form-grid compact-form">
          ...
          <ClassSelectFields programs={data.programs} academicBatches={data.academicBatches} batches={data.batches} defaultProgramId={payment.programId} defaultBatchId={payment.batchId ?? ""} />
