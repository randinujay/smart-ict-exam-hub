...
import { ArrowRight, FileUp, TrendingDown, TrendingUp } from "lucide-react";
...
export default async function AdminResultsPage() {
  ...
  return <div>
    ...
    <section className="results-table-card">
      ...
      <div className="responsive-table"><table><thead><tr><th>Student</th><th>Assessment</th><th>Source</th><th>Status</th><th>Marks</th><th>Percentage</th><th>Date</th><th/></tr></thead><tbody>{ordered.map((result)=>{const student=data.students.find((item)=>item.id===result.studentId);return <tr key={result.id}><td><span className="table-title-cell"><strong>{student?.fullName??"Student"}</strong><span>{student?.phone}</span></span></td><td>{result.assessmentTitle}</td><td><StatusBadge tone={result.source==="school"?"neutral":"brand"}>{result.source==="school"?"School":"Smart ICT"}</StatusBadge></td><td><StatusBadge tone={result.status==="published"?"success":"warning"}>{result.status}</StatusBadge></td><td>{result.obtainedMarks}/{result.totalMarks}</td><td><strong className="score-cell"><TrendingUp size={15}/>{Math.round(result.percentage)}%</strong></td><td>{new Date(result.completedAt).toLocaleDateString("en-LK")}</td><td>{result.status==="pending"?<Link href={`/adminrandinu/results/${result.id}`} className="button button-small button-primary">Review <ArrowRight size={14}/></Link>:<span>—</span>}</td></tr>})}</tbody></table></div>
    </section>
  </div>;
}
