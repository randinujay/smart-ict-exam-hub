...
returns jsonb language sql stable security definer set search_path='' as $$
  select jsonb_build_object(
    ...
    'fileType',r.file_type,'access',r.access_type,'publishedAt',r.published_at,
...
returns jsonb language sql immutable set search_path='' as $$
  select jsonb_build_object(
    'id',r.id,'moduleId',r.module_id,'title',r.title,'description',r.description,
    'videoUrl',r.video_url,'publishedAt',r.published_at,'access',r.access_type,'duration',r.duration_label
  );
$$;
